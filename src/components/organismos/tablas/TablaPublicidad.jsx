...
export function TablaPublicidad({
  SetopenRegistro,
  setdataSelect,
  setAccion,
}) {
  const [pagina, setPagina] = useState(1);
  const { eliminarPublicidad, datapublicidad, buscador, setBuscador } = usePublicidadStore();

  console.log("TablaPublicidad - datapublicidad:", datapublicidad);
  console.log("TablaPublicidad - buscador (from store):", buscador);
  ...
  const editar = async (data) => {
    try {
      ...
      console.log("TablaPublicidad - editar - data recibido:", data);
      console.log("TablaPublicidad - editar - data.video:", data.video);
  ...
  const eliminar = async (p) => {
    try {
      ...
      if (result.isConfirmed) {
        await eliminarPublicidad(p.id);
        // La actualización de la tabla ahora se maneja dentro de eliminarPublicidad
      }
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      minSize: 150,
      cell: (info) => (
        <td data-title="Referencia" className="ContentCell">
          <span className="cell-scrollable">{info.getValue() || "Sin referencia"}</span>
        </td>
      ),
    },
    {
      ...
      minSize: 120,
      cell: (info) => (
        <td data-title="Celular" className="ContentCell">
          <span className="cell-scrollable">{info.getValue() || "Sin celular"}</span>
        </td>
      ),
    },
    {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      ...
      const referencia = row.getValue("referencia")?.toLowerCase() || "";
      const celular = row.getValue("numerocelular")?.toLowerCase() || "";
      
      const match = (
        descripcion.includes(filterValue.toLowerCase()) ||
        referencia.includes(filterValue.toLowerCase()) ||
        celular.includes(filterValue.toLowerCase())
      );
