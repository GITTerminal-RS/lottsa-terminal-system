...
export function ListaModulos({ checkboxs, setCheckboxs, accion, mostrarCartelera, mostrarPersonal = true, disabled = false }) {
  ...
  return (
    <Container>
      {checkboxs?.map((item, index) => {
        if (item.nombre === "Cartelera cultural y turística" && !mostrarCartelera) {
          return null;
        }
        if (item.nombre === "Personal" && !mostrarPersonal) {
          return null;
        }
        return (
          <div className="content" key={index} onClick={() => handlecheckbox(item.id)} style={disabled ? { opacity: 0.6, pointerEvents: 'none' } : {}}>
