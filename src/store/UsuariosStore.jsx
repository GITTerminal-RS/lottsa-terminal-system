...
export const useUsuariosStore = create((set, get) => ({
  ...
  mostrarusuariosTodos: async (p) => {
    ...
    set({ usuariosItemSelect: (response && response[0]) ? response[0] : {} });
    return response || [];
  },
  selectusuarios: (p) => {
  ...
  eliminarusuarios: async (p, idoperadora) => {
    await EliminarUsuarios(p);
    const { mostrarusuariosTodos, mostrarSuperadmins } = get();
    if (!idoperadora) {
      // Si no hay operadora (root), refrescar superadmins
      set(mostrarSuperadmins());
    } else {
      set(mostrarusuariosTodos({ _id_operadora: idoperadora }));
    }
  },
  editarusuarios: async (p, datacheckpermisos, idoperadora) => {
  ...
  mostrarSuperadmins: async () => {
    // Obtiene todos los usuarios con tipouser 'superadmin'
    const { data, error } = await supabase
      .from('usuarios')
      .select()
      .eq('tipouser', 'superadmin');
    if (data) {
